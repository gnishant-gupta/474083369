...
def main():
    ...
    alert_rule_id = extract_action_param(
        siemplify, param_name="Alert Rule ID", is_mandatory=True, print_value=True
    )
    enable_alert_rule = extract_action_param(
        siemplify, param_name="Enable Alert Rule", print_value=True, input_type=bool
    )
    ...
    frequency = extract_action_param(
        siemplify, param_name="Frequency", print_value=True
    )
    lookup_period = extract_action_param(
        siemplify, param_name="Period of Lookup Data", print_value=True
    )
    trigger_operator = extract_action_param(
        siemplify, param_name="Trigger Operator", print_value=True
    )
    trigger_threshold = extract_action_param(
        siemplify, param_name="Trigger Threshold", print_value=True, input_type=int
    )
    enable_suppression = extract_action_param(
        siemplify, param_name="Enable Suppression", print_value=True, input_type=bool
    )
    suppression_duration = extract_action_param(
        siemplify, param_name="Suppression Duration", print_value=True
    )
    description = extract_action_param(
        siemplify, param_name="Description", print_value=True
    )
    tactics = string_to_multi_value(
        extract_action_param(siemplify, param_name="Tactics", print_value=True)
    )

    siemplify.LOGGER.info("----------------- Main - Started -----------------")

    output_message = (
        f"Successfully updated {PRODUCT_NAME} alert rule with ID {alert_rule_id}"
    )
    ...
    try:
        ...
        MicrosoftAzureSentinelManager.validate_alert_rule_severities(
        ...
        output_message = (
            f"Error executing action '{UPDATE_ALERT_RULE_SCRIPT_NAME}'. Reason: {e}"
        )
        result_value = False
