...
from constants import INTEGRATION_NAME, RUN_KQL_QUERY_SCRIPT_NAME, KQL_QUERY_RESULTS
...
def main():
    ...
    query = extract_action_param(
        siemplify, param_name="KQL Query", is_mandatory=True, print_value=True
    )
    timespan = extract_action_param(siemplify, param_name="Time Span", print_value=True)
    timeout = extract_action_param(
        siemplify, param_name="Query Timeout", print_value=True, input_type=int
    )
    limit = extract_action_param(
        siemplify, param_name="Record Limit", print_value=True, input_type=int
    )
    ...
    try:
        ...
        query_result = manager.run_kql_query(
            query=query, timespan=timespan, timeout=timeout, limit=limit
        )
        ...
        if query_result and not query_result.is_empty():
            ...
            siemplify.result.add_data_table(
                ...
                data_table=construct_csv(
                    query_result.to_table(include_empty_tactics=True)
                ),
            )

    except Exception as e:
        output_message = (
            f"Error executing action '{RUN_KQL_QUERY_SCRIPT_NAME}'. Reason: {e}"
        )
