...
from constants import GRAPH_API_ROOT, GRAPH_API_ENDPOINTS
...
from datamodels import CustomHuntingRuleRequest, TagCollection
...
from utils import convert_list_to_comma_separated_string
...
class MicrosoftAzureSentinelManager:
    ...
    def fetch_token(self):
        ...
        raise MicrosoftAzureSentinelManagerError(
            f"Failed fetching token. Error code: {response.status_code}. Description: {response.json().get('error_description', response.content)}"
        )
    ...
    def get_incident_tags(self, incident_id: str) -> list[str]:
        """
        Retrieves a list of tags associated with a Microsoft Sentinel incident.
        """
    ...
    def get_incident_comments(
        ...
        :param incident_number: The ID of the incident to get comments from
        :param start_time: The timestamp to filter comments from
        """
        response: requests.Response = self.session.get(
            self._get_full_url(
                "graph-incident",
    ...
    def get_incidents_by_ids(self, incident_ids: list[str]) -> list[SingleJson]:
        """
    ...
    def validate_login_response(response, error_msg="An error occurred"):
        ...
        try:
            ...
            raise MicrosoftAzureSentinelManagerError(
                f"{error_msg}: {error} {response.json().get('error_description', response.content)}"
            )
